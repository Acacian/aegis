...
RUN pip install --no-cache-dir 'agent-aegis[server]==0.9.0'
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: aegis
          image: your-registry/aegis-server:0.9.0
          ports:
            - containerPort: 8000
          env:
...
- [ ] **Pin versions** -- use exact version (`agent-aegis==0.9.0`), not ranges
